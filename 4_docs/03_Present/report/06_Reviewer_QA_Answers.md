...
**Trả lời:** Em đồng ý đây là **rủi ro số một**, và đó cũng là lý do em **thu hẹp testbed còn 2 ngành (lao động + thuế)** thay vì toàn bộ kho luật VN — gắn metadata hiệu lực ở quy mô 2 ngành là **khả thi và kiểm chứng được** trong 6 tháng, không phải tham vọng "toàn bộ luật". Hướng xử lý: (1) **lấy hiệu lực từ chính văn bản** — điều khoản quy định hết hiệu lực, văn bản thay thế — chứ không đoán; (2) **expert spot-check 5%** + canary regression set sau mỗi cập nhật; (3) hệ có **versioning + rollback** nên sai thì khôi phục được. Em **không giả định metadata hoàn hảo** — đã đưa thành *assumption GĐ1* và một mục *Threats to Validity* riêng. Mở rộng sang ngành khác (cùng với quy mô validate) là future work.
✅ Đã ghi Threats to Validity + GĐ1 + Sectoral Scope (1.4).
