...
\begin{tikzpicture}[
  ...
  step/.style={rectangle, rounded corners, draw, align=center, text width=3.4cm, minimum height=0.8cm},
...
\node[step] (n) {New / amended document};
\node[step, below=of n] (ing) {Segment + tag metadata + embed (upsert)};
\node[step, below=of ing] (sup) {Supersession: old unit $v{=}0$, $t_{\text{end}}$, link};
\node[step, below=of sup] (cg) {Citation-graph delta $\to$ recompute $c_i$ (local)};
